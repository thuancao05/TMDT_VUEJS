...
export default defineComponent({
  setup() {
    ...
    const searchKey = ref(""); // Initialize as an empty string
    ...
    const getProductsWithKeySearch = (keyToSearch) => {
      axios
        .post("http://localhost/TMDT/admin/apiTimKiemSanPham.php", {
          key: keyToSearch,
        })
    ...
    onBeforeRouteUpdate((to, from, next) => {
      if (to.query.key !== from.query.key) {
        // The query parameter has changed, trigger a re-render
        getProductsWithKeySearch(to.params.id);
      }
      next();
    });
